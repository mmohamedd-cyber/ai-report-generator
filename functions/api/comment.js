export async function onRequestPost(context) {
  try {
    ...
    if (studentFirstName.length > 40) return json({ error: "Bad name" }, 400);
    ...
    if (!resp.ok) {
      return json({ error: "OpenAI error", detail: data }, resp.status);
    }

    const comment = extractText(data);
    return json({ comment: stripDigits(comment).trim() });
